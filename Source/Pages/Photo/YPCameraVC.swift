...
public class YPCameraVC: UIViewController, UIGestureRecognizerDelegate, YPPermissionCheckable {
    ...
    let photoCapture = newPhotoCapture()
    let v: YPCameraView!
    internal var selection: [YPMediaItem] = []
    override public func loadView() { view = v }
    ...
    func shoot() {
        ...
        photoCapture.shoot { imageData in
            ...
            let img = YPMediaItem.photo(p: YPMediaPhoto(image: noOrietationImage.resizedImageIfNeeded(), fromCamera: true))
            self.selection.append(img)
    ...
    public func selectedMedia(multipleItemsCallback: @escaping (_ items: [YPMediaItem]) -> Void) {
        self.photoCapture.stopCamera()
        
        DispatchQueue.global(qos: .userInitiated).async {
            DispatchQueue.main.async {
                multipleItemsCallback(self.selection)
            }
        }
    }
